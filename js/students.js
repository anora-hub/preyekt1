...
async function getData(content) {
    try {
     ...
     <div class="border-[1px] border-[black]/30 max-w-[300px] w-full h-[440px] rounded-[20px] mr-auto ml-auto block ">
                            ...
                            <div class=" flex gap-[5px] ml-[auto]  mr-[auto] block ">
                            <button
                                class="border border-[black]/20  bg-[black]/20  px-[20px] rounded-[10px]">${el.science.name}</button>
                          <h1>${el.age}</h1>
                                </div>
...
async function addTeacher(payload) {
    try {
        await axios.post("https://69207def31e684d7bfcd401b.mockapi.io/students", payload)
...
form.addEventListener("submit", async function (e) {
    ...
    if (selected) {
        await axios.put(`https://69207def31e684d7bfcd401b.mockapi.io/students/${selected}`, {
            avatar: form[0].value,
...
async function deleteTeacher(id) {
    try {
        await axios.delete(`https://69207def31e684d7bfcd401b.mockapi.io/students/${id}`)
        getData(tearcherCards);
        localStorage.setItem("tearcherlar", JSON.stringify(tearcherlar))
...
async function edit(id) {
    try {
        ...
        let res = await axios.get(`https://69207def31e684d7bfcd401b.mockapi.io/students/${id}`);
